...
const { parseMdTable, parseMdTableForMultipleStruct } = require('./md-utils')
...
async function postmanToOpenApi (input, output, {
  ...
  responseHeaders = true, replaceVars = false, additionalVars = {}, values = []
} = {}) {
  // TODO validate?
  let collectionFile = await readFile(input, 'utf8')
  if (replaceVars || values.length !== 0) {
    collectionFile = replacePostmanVariables(collectionFile, additionalVars, values)
  }
...
function descriptionParse (description) {
  ...
  return {
    ...
    requestBodyMeta: parseMdTableForMultipleStruct(splitDesc[1])
  }
}
